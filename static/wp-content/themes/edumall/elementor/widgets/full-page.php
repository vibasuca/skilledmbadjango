...
class Widget_Full_Page extends Base {
	...
	public function get_title() {
		return esc_html__( 'Fullpage', 'edumall' );
	}
	...
	private function add_slides_section() {
		...
		$repeater->start_controls_tab( 'repeater_slides_content_tab', [ 'label' => esc_html__( 'Content', 'edumall' ) ] );
		...
		$repeater->add_control( 'anchor', [
			'label' => esc_html__( 'Anchor', 'edumall' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$repeater->add_control( 'skin', [
			'label'       => esc_html__( 'Skin', 'edumall' ),
			'description' => esc_html__( 'Controls the skin of pagination & header when view this section.', 'edumall' ),
			'type'        => Controls_Manager::CHOOSE,
			'options'     => [
				'light' => [
					'text' => esc_html__( 'Light', 'edumall' ),
				],
				'dark'  => [
					'text' => esc_html__( 'Dark', 'edumall' ),
				],
			],
			'toggle'      => false,
			'default'     => 'light',
		...
		$repeater->add_control( 'auto_height', [
			'label' => esc_html__( 'Auto Height', 'edumall' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$repeater->end_controls_tab();

		$repeater->start_controls_tab( 'repeater_slides_background_tab', [ 'label' => esc_html__( 'Background', 'edumall' ) ] );
		...
		$this->add_control( 'slides', [
			...
			'default'     => [
				[
					'title' => esc_html__( 'Slide #1', 'edumall' ),
				],
				[
					'title' => esc_html__( 'Slide #2', 'edumall' ),
				],
			],
			'title_field' => '{{{ title }}}',
		] );
	...
	private function add_fullpage_options_section() {
		$this->start_controls_section( 'fullpage_options_section', [
			'label' => esc_html__( 'Fullpage Options', 'edumall' ),
		] );

		$this->add_control( 'dots', [
			'label' => esc_html__( 'Dots Enable', 'edumall' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'dots_alignment', [
			'label'     => esc_html__( 'Dots Alignment', 'edumall' ),
			'type'      => Controls_Manager::CHOOSE,
			'options'   => [
				'left'  => [
		...
		$this->add_control( 'numbers', [
			'label' => esc_html__( 'Numbers Enable', 'edumall' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'sharing', [
			'label' => esc_html__( 'Share Enable', 'edumall' ),
			'type'  => Controls_Manager::SWITCHER,
		] );
	...
	private function print_slide_numbers( array $settings ) {
		...
		<div id="full-page-numbers" class="full-page-numbers">
			...
			<div class="title">
				<?php esc_html_e( 'Projects', 'edumall' ); ?>
			</div>
		</div>
		<?php
	}
