...
class Widget_Button extends Base {
	...
	public function get_title() {
		return esc_html__( 'Advanced Button', 'edumall' );
	}
	...
	private function add_button_settings_section() {
		...
		$this->add_control( 'text', [
			...
			'default'     => esc_html__( 'Click here', 'edumall' ),
			'placeholder' => esc_html__( 'Click here', 'edumall' ),
		] );
		...
		$this->add_control( 'action', [
			...
			'description' => esc_html__( 'To make smooth scroll action work then input button link like this: #about-us-section.', 'edumall' ),
			'type'        => Controls_Manager::SELECT,
			'default'     => '',
			'options'     => [
				''              => esc_html__( 'Normal', 'edumall' ),
				'smooth_scroll' => esc_html__( 'Smooth scroll to a section', 'edumall' ),
				'popup_video'   => esc_html__( 'Open Popup Video', 'edumall' ),
			],
		] );
		...
		$this->add_control( 'button_css_class', [
			'label'       => esc_html__( 'Button Class', 'edumall' ),
			'type'        => Controls_Manager::TEXT,
			'dynamic'     => [
				'active' => true,
			],
			'default'     => '',
			'title'       => esc_html__( 'Add your custom classes WITHOUT the dot key. e.g: my-class', 'edumall' ),
			'label_block' => false,
		] );
	...
	private function add_button_badge_section() {
		...
		$this->add_control( 'badge_text', [
			...
			'placeholder' => esc_html__( '$59', 'edumall' ),
		] );
	...
	private function add_skin_section() {
		...
		$this->add_control( 'hover_animation', [
			'label' => esc_html__( 'Hover Animation', 'edumall' ),
