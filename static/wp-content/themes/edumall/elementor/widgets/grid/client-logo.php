...
class Widget_Client_Logo extends Static_Grid {
	...
	public function get_title() {
		return esc_html__( 'Client Logo', 'edumall' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control( 'hover', [
			'label'   => esc_html__( 'Hover Type', 'edumall' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''          => esc_html__( 'None', 'edumall' ),
				'grayscale' => esc_html__( 'Grayscale to normal', 'edumall' ),
				'opacity'   => esc_html__( 'Opacity to normal', 'edumall' ),
				'faded'     => esc_html__( 'Normal to opacity', 'edumall' ),
			],
			'default' => '',
		] );
			...
			<a <?php $this->print_render_attribute_string( $item_link_key ); ?>>
				...
				<div class="image">
					<img src="<?php echo esc_url( $item['logo']['url'] ); ?>"
					     alt="<?php esc_attr_e( 'Client Logo', 'edumall' ); ?>">
				</div>
