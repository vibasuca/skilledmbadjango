...
class Widget_Twitter extends Base {
	...
	public function get_title() {
		return esc_html__( 'Twitter', 'edumall' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control( 'consumer_key', [
			'label' => esc_html__( 'Consumer Key', 'edumall' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'consumer_secret', [
			'label' => esc_html__( 'Consumer Secret', 'edumall' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'access_token', [
			'label' => esc_html__( 'Access Token', 'edumall' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'access_token_secret', [
			'label' => esc_html__( 'Access Token Secret', 'edumall' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'username', [
			'label' => esc_html__( 'Twitter Username', 'edumall' ),
			'type'  => Controls_Manager::TEXT,
		] );
		...
		$this->add_control( 'show_date', [
				...
				'label_on'     => esc_html__( 'Yes', 'edumall' ),
				'label_off'    => esc_html__( 'No', 'edumall' ),
				'return_value' => '1',
			]
		);
