...
class Widget_Attribute_List extends Base {
	...
	public function get_title() {
		return esc_html__( 'Attribute List', 'edumall' );
	}
	...
	protected function register_controls() {
		...
		$repeater->add_control( 'name', [
			...
			'default'     => esc_html__( 'Name', 'edumall' ),
			'label_block' => true,
		] );
		...
		$this->add_control( 'items', [
			...
			'default'     => [
				[
					'name'  => esc_html__( 'Attribute #1', 'edumall' ),
					'value' => esc_html__( 'Value #1', 'edumall' ),
				],
				[
					'name'  => esc_html__( 'Attribute #2', 'edumall' ),
					'value' => esc_html__( 'Value #2', 'edumall' ),
				],
			],
			'title_field' => '{{{ name }}}',
		] );
					...
					<?php if ( $settings['items'] && count( $settings['items'] ) > 0 ) {
						foreach ( $settings['items'] as $key => $attribute ) {
							...
							<div class="item">
								...
								<div
									class="value"><?php echo wp_kses( $attribute['value'], 'edumall-default' ); ?></div>
							</div>
							<?php
						}
					}
